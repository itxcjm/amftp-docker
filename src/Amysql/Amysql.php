...
class Amysql {
	...
	public function __construct() 
	{
		...
		($Config['DebugPhp'] && error_reporting(E_ALL & ~E_NOTICE)) || error_reporting(0);						// 是否报错
	...
	static public function filter(&$array, $function) 
	{
		...
		foreach ($array as $key => $value) 
			(is_array($value) && $array[$key] = Amysql::filter($value, $function)) || (is_string($value) && $array[$key] = $function($value));
		Return $array;
	}
}
...
class AmysqlModel {
	...
	public function _query($sql) 
	{
		...
		$this -> Affected = mysqli_affected_rows($this -> MysqlConnect);
		$this -> QueryStatus = '(ok)';
		Return $result;
	}
	...
	public function _insert($table, $data)
	{
		...
		$this -> _query($sql);
		$this -> InsertId = mysqli_insert_id($this -> MysqlConnect);
...
class DB
{
	...
	{
		$this -> DBS[$Config['ModelTag']] = @mysqli_connect($Config['Host'], $Config['User'], $Config['Password']);
		(!$this -> DBS[$Config['ModelTag']] && Amysql::AmysqlNotice(mysqli_connect_error() . ' MySQL连接出错。'));

		(!empty($Config['DBname']) && mysqli_select_db($this -> DBS[$Config['ModelTag']], $Config['DBname']));
